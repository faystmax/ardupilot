...
static void update_GPS(void)
{
    ...
    if (gps_updated) {
        ...
        if (ap.home_is_set) {
            telem.getGpsGlitch().check_position();
            report_gps_glitch = (telem.getGpsGlitch().glitching() && !ap.usb_connected && hal.util->safety_switch_state() != AP_HAL::Util::SAFETY_DISARMED);
